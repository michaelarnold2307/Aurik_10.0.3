#!/usr/bin/env python3
"""
BW Reconstructor V4 — GAN-Training auf MUSDB18 (echte Musik).

Architektur = V3 (U-Net + FiLM + PatchGAN)
Daten = MUSDB18-HQ train split
Neu: Echte Musik statt synthetischer Töne.
"""
...
N_MELS = 128
N_FRAMES = 128
SEGMENT_SEC = 6.0  # seconds per training segment
...
class MUSDB18Dataset(torch.utils.data.Dataset):
    """Lädt MUSDB18-HQ Mixdowns und erzeugt bandbreitenbegrenzte Paare."""

    def __init__(self, data_dir: str, split: str = "train", segment_sec: float = 6.0):
        self.data_dir = Path(data_dir) / split
        self.segment_samples = int(segment_sec * SR)
        self.tracks = sorted(self.data_dir.glob("*/mixture.wav"))
        if not self.tracks:
            raise FileNotFoundError(f"Keine mixture.wav in {self.data_dir} gefunden")
        print(f"MUSDB18 {split}: {len(self.tracks)} tracks, {segment_sec}s segments")

    def __len__(self):
        return len(self.tracks) * 10  # 10 random segments per track per epoch
    ...
    def __getitem__(self, idx):
        ...
        # Lade Mixdown
        audio, file_sr = torchaudio.load(str(track_path))
        audio = audio.mean(0)  # Mono

        if file_sr != SR:
            audio = torchaudio.functional.resample(audio, file_sr, SR)

        # Brauche mindestens segment_samples + 1s für random crop
        needed = self.segment_samples + SR
        if len(audio) < needed:
            # Pad if track is too short
        ...
        audio_limited = _butter_lp(audio_seg, cutoff, SR).astype(np.float32)

        return (
            torch.from_numpy(audio_limited),
            torch.from_numpy(audio_seg.astype(np.float32)),
            torch.tensor(cutoff / 11025.0, dtype=torch.float32),
        )


def _butter_lp(y, cutoff, sr, order=6):
    nyq = sr / 2
    if cutoff >= nyq * 0.98:
        return y.copy()
...
def train(data_dir="models/musdb18hq", epochs=300, batch_size=6, lr=1e-4, base_ch=24):
    device = torch.device("cpu")
    print(f"Device: {device}")
    print(f"Data:   {data_dir}")

    dataset = MUSDB18Dataset(data_dir, split="train", segment_sec=SEGMENT_SEC)
    ...
    print(f"Generator: {n_g:,} params | Discriminator: {n_d:,} params | Total: {n_g+n_d:,}")

    # Load V3 checkpoint (transfer learning)
    ...
    if ckpt_path.exists():
        ...
        print(f"Loaded {len(pretrained)}/{len(g_state)} generator weights from V3 checkpoint")
    ...
    lambda_stft = 0.3
    lambda_gan_end = 0.1
    steps_per_epoch = min(300, len(dataloader))

    print(f"Epochs: {epochs}, Batch: {batch_size}, Steps/epoch: {steps_per_epoch}")
    print(f"LR: {lr}, Hinge-GAN + MUSDB18 real music")
    print()
    ...
    for epoch in range(epochs):
        ...
        gan_weight = lambda_gan_end * min(1.0, epoch / 50)  # ramp up over 50 epochs
        ...
        for step, (x_audio, y_audio, cutoff_norm) in enumerate(dataloader):
            ...
            cutoff_norm = cutoff_norm.to(device).unsqueeze(1)  # (B,)->(B,1)
...
if __name__ == "__main__":
    ...
    p.add_argument("--data", type=str, default="models/musdb18hq")
    p.add_argument("--epochs", type=int, default=300)
    p.add_argument("--batch-size", type=int, default=6)
    p.add_argument("--lr", type=float, default=1e-4)
    p.add_argument("--base-ch", type=int, default=24)
    args = p.parse_args()
    train(args.data, args.epochs, args.batch_size, args.lr, args.base_ch)
