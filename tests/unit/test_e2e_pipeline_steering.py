...
class TestE2EPipeline:
    ...
    def test_01_naturalness_optimizer_improves_hpe(self):
        ...
        assert result.hpe_after >= result.hpe_before - 0.01,             f"HPE verschlechtert: {result.hpe_before:.3f} → {result.hpe_after:.3f}"
    ...
    def test_07_multiple_materials_all_improve(self):
        ...
        for material in ["vinyl", "tape", "cd_digital", "mp3_low", "unknown"]:
            ...
            assert result.hpe_after >= result.hpe_before - 0.02,                 f"{material}: HPE {result.hpe_before:.3f} → {result.hpe_after:.3f}"
    ...
    def test_08_glue_stage_applied(self):
        ...
        audio = _make_defective_audio()
        result = optimize_naturalness(audio, audio.copy(), _SR)
        assert "multiband_glue" in result.applied_stages or result.glue_reduction_db > 0,             "Glue-Stage wurde nicht angewendet!"
