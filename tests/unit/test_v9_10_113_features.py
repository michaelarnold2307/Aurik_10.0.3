...
class TestPhase40LufsStudio2026:
    ...
    def test_restoration_condition_targets_correct_modes(self):
        ...
        src = self._get_source()
        assert '"restoration"' in src or "'restoration'" in src, (
            "restoration-Modus im LUFS-Delta-Cap nicht erwähnt"
        )
...
class TestPhase09SeverityAdaptiveBlend:
    ...
    def test_moderate_severity_threshold(self):
        ...
        src = self._get_source()
        assert '0.35' in src, (
            "Grenzwert 0.35 für moderate Crackle-Severity fehlt in phase_09."
        )
    ...
    def test_heavy_severity_threshold(self):
        ...
        src = self._get_source()
        assert '0.60' in src, (
            "Grenzwert 0.60 für schwere Crackle-Severity fehlt in phase_09."
        )
...
class TestPhase29HFGFloor:
    ...
    def test_hf_floor_enhancement_in_source(self):
        ...
        src = self._get_source()
        assert '"presence"' in src or "'presence'" in src, (
            "Presence-Zone in phase_29 nicht erwähnt"
        )
        assert '"air"' in src or "'air'" in src, (
            "Air-Zone in phase_29 nicht erwähnt"
        )
        assert '0.45' in src, (
    ...
    def test_hf_floor_computed_correctly(self):
        ...
        assert abs(_hf_floor - expected) < 1e-6, (
            f"TAPE HF-G_floor: erwartet {expected:.4f}, erhalten {_hf_floor:.4f}"
        )
...
class TestPhase55AdaptiveAROrder:
    ...
    def test_long_gap_threshold_2400_samples(self):
        ...
        src = self._get_source()
        assert '2400' in src, (
            "Schwellenzeit 2400 Samples (≈ 50 ms @ 48 kHz) für AR-Order-Erhöhung fehlt in phase_55."
        )
    ...
    def test_higher_ar_order_192(self):
        ...
        src = self._get_source()
        assert '192' in src, (
            "AR-Order 192 fehlt in phase_55. Für Gaps > 50 ms ist AR(192) nötig."
        )
    ...
    def test_adaptive_ar_logic_short_gap(self):
        ...
        _ar_order_adaptive = (
            min(192, max(16, left_ctx_len - 1)) if gap_len > 2400 else _AR_ORDER
        )
    ...
    def test_adaptive_ar_logic_long_gap(self):
        ...
        assert _ar_order_adaptive == 192, (
            f"Lange Lücke: AR-Order sollte 192 sein, ist {_ar_order_adaptive}"
        )
