...
def test_path_normalization_absolute_vs_relative():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert normalized_abs == normalized_rel, (
                f"Path normalization failed: {normalized_abs} != {normalized_rel}"
            )
        finally:
...
def test_path_normalization_trailing_slashes():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        norm_double = os.path.normpath(os.path.realpath(path_with_double_slash))
...
def test_pre_analysis_handover_path_matching_logic():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        _pre_file_norm = os.path.normpath(os.path.realpath(_latest_pre_analysis_file)) if _latest_pre_analysis_file else ""
