...
def test_04_no_rollback_on_improvement():
    ...
    result_audio, rolled_back = guard.check_after_phase(
        state, "phase_03_denoise", audio, improved, SR,
    )
...
def test_05_rollback_on_cumulative_drift():
    ...
    result_audio, rolled_back = guard.check_after_phase(
        state, "phase_03_denoise", bad_audio, degraded, SR,
    )
    assert rolled_back
...
def test_06_consecutive_rollbacks_stop_pipeline():
    from backend.core.cumulative_interaction_guard import get_interaction_guard, MAX_CONSECUTIVE_ROLLBACKS
    guard = get_interaction_guard()
    state = guard.reset()
    audio = _audio()
    guard.set_pre_pipeline_baseline(state, audio, _goals(nat=0.92))
...
def test_08_critical_pair_denoise_dereverb():
    ...
    _, rolled_back = guard.check_after_phase(
        state, "phase_20_reverb_reduction", audio * 0.8, degraded, SR,
    )
    assert rolled_back
...
def test_09_critical_pair_no_trigger_if_good():
    ...
    _, rolled_back = guard.check_after_phase(
        state, "phase_20_reverb_reduction", audio, good, SR,
    )
    assert not rolled_back
...
def test_10_stft_phase_tracking():
    ...
    audio = _audio(2.0)
    guard.set_pre_pipeline_baseline(state, audio, _goals())
    # Execute 3 STFT phases
    for pid in ["phase_03_denoise", "phase_07_harmonic_restoration", "phase_29_tape_hiss_reduction"]:
...
def test_14_p1_p2_goals_constant():
    ...
    assert "naturalness" in P1_P2_GOALS
    assert "authenticity" in P1_P2_GOALS
    assert "tonal_center" in P1_P2_GOALS
    assert "timbre" in P1_P2_GOALS
    assert "articulation" in P1_P2_GOALS
...
def test_20_is_better_checkpoint_worse():
    ...
    state.best_checkpoint = InteractionGuardCheckpoint(
        audio=_audio(), phase_id="best", goal_scores=_goals(nat=0.95),
    )
    # Worse goals
...
def test_22_should_stop_prevents_further_checks():
    ...
    audio = _audio()
    result_audio, rolled_back = guard.check_after_phase(
        state, "phase_03_denoise", audio, _goals(), SR,
    )
    assert not rolled_back
