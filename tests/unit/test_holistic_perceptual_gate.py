...
def test_32_update_rejected_if_hpi_too_low():
    ...
    gate.update_reference_memory(
        audio, SR, hpi=0.4, artifact_freedom=1.0, p1_p2_passed=True,
        genre="jazz", material="vinyl_std", era_bin="pre-1960",
    )
...
def test_33_update_rejected_if_artifact_freedom_low():
    ...
    audio = _audio(2.0)
    gate.update_reference_memory(
        audio, SR, hpi=0.8, artifact_freedom=0.90, p1_p2_passed=True,
        genre="jazz", material="vinyl_std", era_bin="pre-1960",
    )
    assert ("jazz", "vinyl_std", "pre-1960") not in gate._ref_memory
...
def test_34_update_rejected_if_p1_p2_failed():
    ...
    audio = _audio(2.0)
    gate.update_reference_memory(
        audio, SR, hpi=0.8, artifact_freedom=1.0, p1_p2_passed=False,
        genre="jazz", material="vinyl_std", era_bin="pre-1960",
    )
    assert ("jazz", "vinyl_std", "pre-1960") not in gate._ref_memory
...
def test_35_update_creates_entry():
    ...
    audio = _audio(2.0)
    gate.update_reference_memory(
        audio, SR, hpi=0.8, artifact_freedom=1.0, p1_p2_passed=True,
        genre="jazz", material="vinyl_std", era_bin="pre-1960",
    )
...
def test_36_update_ema_three_times_calibrated():
    ...
    for _ in range(3):
        gate.update_reference_memory(
            audio, SR, hpi=0.9, artifact_freedom=1.0, p1_p2_passed=True,
            genre="pop", material="tape_std", era_bin="1960-1990",
        )
    assert key in gate._ref_memory
...
def test_37_ema_smoothing_alpha():
    ...
    from backend.core.holistic_perceptual_gate import HolisticPerceptualGate, _EMA_ALPHA
    gate = HolisticPerceptualGate()
    audio1 = _audio(2.0, freq=440.0)
    audio2 = _audio(2.0, freq=880.0)  # different embedding

    gate.update_reference_memory(
        audio1, SR, hpi=0.9, artifact_freedom=1.0, p1_p2_passed=True,
        genre="rock", material="digital", era_bin="post-1990",
    )
    emb_after_first = gate._ref_memory[("rock", "digital", "post-1990")].embedding.copy()

    gate.update_reference_memory(
        audio2, SR, hpi=0.9, artifact_freedom=1.0, p1_p2_passed=True,
        genre="rock", material="digital", era_bin="post-1990",
    )
    ...
    corr = float(np.dot(emb_after_first, emb_after_second) / (
...
def test_39_fallback_stufe1_exact_match():
    ...
    gate.update_reference_memory(
        audio, SR, hpi=0.9, artifact_freedom=1.0, p1_p2_passed=True,
        genre="blues", material="shellac", era_bin="pre-1960",
    )
...
def test_40_fallback_stufe2_same_era_same_genre():
    ...
    gate.update_reference_memory(
        audio, SR, hpi=0.9, artifact_freedom=1.0, p1_p2_passed=True,
        genre="jazz", material="shellac", era_bin="pre-1960",
    )
...
def test_41_fallback_stufe3_same_genre():
    ...
    gate.update_reference_memory(
        audio, SR, hpi=0.9, artifact_freedom=1.0, p1_p2_passed=True,
        genre="jazz", material="tape_std", era_bin="1960-1990",
    )
...
def test_42_reference_used_in_hpi_low_restorability():
    ...
    gate.update_reference_memory(
        audio, SR, hpi=0.9, artifact_freedom=1.0, p1_p2_passed=True,
        genre="folk", material="shellac", era_bin="pre-1960",
    )
...
def test_45_genre_material_era_in_detail():
    ...
    result = gate.evaluate_restoration(
        audio, audio, SR,
        genre="schlager", material="vinyl_std", era_bin="1960-1990",
    )
