...
import numpy as np
from backend.core.phases.phase_interface import PhaseInterface, PhaseMetadata, PhaseCategory
...
def test_surgical_dispatch_skip_tiny():
    sr = 48000; audio = np.random.randn(sr).astype(np.float32)*0.1; orig = audio.copy()
    result = PhaseInterface.surgical_dispatch(_MockPhase(), audio, sr, "vinyl", time_ranges=[(0.5, 0.50001)], context_ms=0)
    assert np.allclose(result, orig, atol=1e-7)

def test_surgical_dispatch_mono():
    sr = 48000; audio = np.random.randn(sr).astype(np.float32)*0.1
...
class _SpikePhase:
    ...
    def get_metadata(self): return self._metadata
    def process(self, audio, sample_rate, material_type="unknown", **kwargs): return audio * 100.0

def test_surgical_dispatch_clamp():
    sr = 48000; audio = np.random.randn(sr).astype(np.float32)*0.01
