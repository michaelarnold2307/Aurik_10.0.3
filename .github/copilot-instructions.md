...
Restaurations-Ziel: Kein hörbarer Eingriff. Gates: `OQS ≥ 80`, `timbral_fidelity ≥ 0.93` zum best_carrier_checkpoint, Musical Noise ≤ Trägerprofil, Frisson-Zonen vollständig erhalten. Aurik zeigt dem Hörer nie ein Ergebnis, das technisch besser aussieht aber schlechter klingt.

### §0g [RELEASE_MUST] Autonomes Entscheidungs-Doktrin
**Aurik trifft alle Entscheidungen autonom.** Kaskade: Erkennen (MediumDetector+EraClassifier+DefectScanner) → Planen (GPOptimizer+PhaseConductor) → Ausführen (UV3, Pre/Post-Messung) → Validieren (PMGG+CIG+AFG+HPI) → Exportieren (nur wenn HPI > 0 + artifact_freedom ≥ 0.95). **VERBOTEN**: Hartkodierte song-spezifische Entscheidungen, Strength-Konstanten ohne `compute_adaptive_drift_tolerance()`.
...
| **Stem-Enhancement** | **VERBOTEN** | Vollständige Enhancement-Kette (§1.5) |
...
| Pipeline läuft nach `_mas_fully_achieved=True` weiter | MAS-Erreichung → sofortiger Stop (weitere Phasen = Over-Processing) | §2.65 |
| Gesangsmaterial ohne VQI-Messung exportiert | `compute_vqi()` wenn `panns_singing ≥ 0.35`; `VQI < 0.72` → Recovery-Kaskade | §2.35c |
...
`artifact_freedom < 0.95` → Gate-Fail (einziger Veto-Faktor). VERSA primär (`use_versa_in_loop=True`); MERT nur Fallback; `MERT_floor = max(raw_mert, 0.5)`.
